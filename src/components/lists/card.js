...
const Card = memo(({ item }) => {
    ...
    return (
        <Pressable onPress={() => { navigation.navigate('MangaDetails', {id: item.id });}} style={{ backgroundColor: "#303030", borderRadius: 6, width: 162, marginRight: 16,   justifyContent: 'center', }}>
            <Image source={{ uri: item.capa }} style={{ width: 102, height: 152, borderRadius: 6, alignSelf: 'center', marginBottom: 6, zIndex: 99,}} />
            ...
            <LinearGradient colors={['transparent', '#303030']} style={{ flexGrow: 1, height: 60,  marginTop: -45,}} />
            <Column style={{ padding: 12, }}>
                <Title style={{ fontSize: 16, marginTop: 20,}}>{item?.name.slice(0,40)}</Title>
